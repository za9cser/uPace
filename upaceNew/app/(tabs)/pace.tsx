...
export default function PaceCalculatorScreen() {
  ...
  const [lapDistances, setLapDistances] = useState<LapDistance[]>([{ id: '1', value: '' }]);
  ...
  const [snackbar, setSnackbar] = useState({ visible: false, message: '', type: 'info' as 'info' | 'success' | 'warning' | 'error' });

  const showSnackbar = (message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info') => {
  ...
  const calculateMissingValue = (values: PaceFormValues): PaceFormValues => {
    const timeSeconds = values.time && values.time.trim() ? parseTime(values.time) : null;
    const paceSeconds = values.pace && values.pace.trim() ? parsePace(values.pace) : null;
    const distance = values.distance && values.distance.trim() ? parseFloat(values.distance) : null;

    const filledCount = [timeSeconds, paceSeconds, distance]
      .filter((v) => v !== null && v > 0)
      .length;
    ...
    } else if (timeSeconds !== null && distance !== null && paceSeconds === null) {
      const calculatedPace = timeSeconds / distance;
      newValues.pace = formatPace(calculatedPace);
    } else if (paceSeconds !== null && distance !== null && timeSeconds === null) {
  ...
  const handleFieldChange = (field: keyof PaceFormValues, value: string) => {
    ...
    if (filledCount >= 2) {
      ...
      if (!newValues.distance.trim() && calculated.distance !== newValues.distance) {
  ...
  const addLap = () => {
    setLapDistances([...lapDistances, { id: Date.now().toString(), value: '' }]);
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={["top"]}>
      ...
      >
        ...
        <Card style={styles.card}>
          <Card.Content>
            <Text variant="titleLarge" style={{ color: theme.colors.text, marginBottom: 12 }}>
              Lap Distances (km)
            </Text>
            <View style={styles.lapContainer}>
              ...
              <Button mode="outlined" onPress={addLap} style={styles.addLapButton}>
                + Add Lap
              </Button>
              <Button
                mode="contained"
          ...
          <Card style={styles.card}>
            <Card.Content>
              <View style={styles.tableHeader}>
                ...
                <Button mode="text" onPress={copyTableToClipboard} textColor={theme.colors.primary}>
                  Copy
                </Button>
              </View>
              <DataTable>
